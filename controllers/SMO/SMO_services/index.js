...
const SMO_ORIGINAL_URL = "https://smoapi.petrolimex.com.vn";
...
const updateInfoSAP = (req, res) => {
  ...
  axios
    ...
    .catch((error) => {
      ...
      res
        .status(500)
        .json(error.response?.data || { message: "Request error" });
    });
};

const loginSMO = async (req, res) => {
  const username = process.env.SMO_USERNAME;
  const password = process.env.SMO_PASSWORD;
